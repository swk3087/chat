...
export async function POST(req: Request) {
  ...
  const existing = await prisma.user.findFirst({ where: { username, NOT: { email: session.user.email } } });
  if (existing) return NextResponse.json({ message: "username taken" }, { status: 409 });
  await prisma.user.update({ where: { email: session.user.email }, data: { displayName, username, image } });
